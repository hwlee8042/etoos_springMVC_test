...
import com.etoos.study.domain.RestSampleVO;
import com.etoos.study.domain.Ticket;
...
public class RestSampleController {
	...
	public RestSampleVO getSample2() {
	...
	public ResponseEntity<RestSampleVO> check(Double height, Double weight){
		...
		RestSampleVO rsvo = new RestSampleVO(0, ""+height, ""+weight);
		
		ResponseEntity<RestSampleVO> result = null;
		
		if(height < 150) {
			result = ResponseEntity.status(HttpStatus.BAD_GATEWAY).body(rsvo);
		}else {
			result = ResponseEntity.status(HttpStatus.OK).body(rsvo);
		}
